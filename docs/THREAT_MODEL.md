...
| T-06 | Crash occurs while a simulated position is open | Atomic local recovery journal, strict schema, partial-fill recovery test, audit/projection checks, and `SimulatedProtectionEvidence` | Rehearsal readiness is never exchange evidence; actual exchange confirmation is deferred to Phase 14. |
...
2. A nonzero simulated position must have `REHEARSAL_READY` simulated stop and reduce-only intent evidence to be locally recovered; otherwise the result hard-halts. This evidence cannot satisfy `ExchangeProtectionEvidence` or the future live gate.
